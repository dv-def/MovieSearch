...
private const val HISTORY_SETTINGS_KEY = "HISTORY_SETTINGS_KEY"
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        activity?.let {
            binding.fragmentSettingsCheckboxHistory.isChecked =
                it.getPreferences(Context.MODE_PRIVATE).getBoolean(HISTORY_SETTINGS_KEY, false)
        }
        ...
        binding.fragmentSettingsBtnSave.setOnClickListener {
            activity?.let {
                it.getPreferences(Context.MODE_PRIVATE)
                    .edit()
                    .putBoolean(HISTORY_SETTINGS_KEY, binding.fragmentSettingsCheckboxHistory.isChecked)
                    .apply()
